...
export class EdgeFunctionsRegistryImpl {
    ...
    projectDir;
    command;
    constructor({ aiGatewayContext, bundler, command, config, configPath, env, featureFlags, getUpdatedConfig, importMapFromTOML, projectDir, publishDir, runIsolate, servePath, watchIgnore, deployEnvironment, }) {
    ...
    static getEnvironmentVariables(envConfig, deployEnvironment) {
    ...
    async setupWatcherForDirectory() {
        const toIgnoredRegex = (dir) => new RegExp(`^${dir.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(/|$)`);
        const toIgnoredEntry = (p) => {
            try {
